  ...
  const submit = async () => {
    ...
    try {
      ...
      const { data: row, error } = await supabase.from("listings").insert({
        seller_id: user!.id,
        kind, title, description, price_points: price, category: category || null,
        ...
        <div className="space-y-4 rounded-2xl border border-border bg-card p-6 shadow-card">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input type="number" min={0} max={100000} value={price} onChange={(e) => setPrice(parseInt(e.target.value) || 0)} />
            </div>
            <div>
