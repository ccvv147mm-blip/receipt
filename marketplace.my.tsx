...
import { Coins, Store, Loader2, Package, Wrench, Plus, BarChart3, Eye, Trash2 } from "lucide-react";
  ...
  const listings = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("listings")
        .select("id, kind, title, description, price_points, category, images, status, created_at")
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-5xl px-4 py-6">
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-border bg-card p-4 shadow-card">
            <div className="flex items-center gap-2 text-muted-foreground"><Coins className="h-4 w-4 text-amber-500" /><span className="text-xs">متوسط السعر</span></div>
            <p className="mt-1 text-2xl font-bold">
              {totalCount > 0 ? Math.round((listings.data?.reduce((s, l) => s + l.price_points, 0) ?? 0) / totalCount) : 0}
            </p>
          </div>
        </div>
              ...
              <div className="p-3">
                ...
                <div className="mt-2 flex items-center justify-between">
                  <div className="flex items-center gap-1 text-amber-500">
                    ...
                    <span className="font-bold">{l.price_points}</span>
