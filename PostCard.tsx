...
import { textToSpeech } from "@/lib/voice.functions";
...
export function PostCard({ post }: { post: FeedPost }) {
  ...
  const speakPost = async () => {
    ...
    try {
      setSpeaking(true);
      const { audioBase64, mime } = await ttsFn({ data: { text: post.content.slice(0, 1500) } });
      stopAllMedia();
      const a = new Audio(`data:${mime || "audio/mpeg"};base64,${audioBase64}`);
      a.volume = 1.0;
      // تعزيز مستوى الصوت عبر Web Audio (أعلى من الحد الافتراضي)
      try {
      ...
      audioRef.current = a;
      registerDetachedAudio(a, () => { audioRef.current = null; setSpeaking(false); });
      a.onended = () => setSpeaking(false);
      await a.play();
    } catch (e) { setSpeaking(false); toast.error(friendlyError(e, "تعذّر تشغيل الصوت")); }
  };
  ...
  const name = post.author?.full_name || post.author?.username || "مستخدم";
  const initial = name.charAt(0);
  const isMine = user?.id === post.author_id;

  return (
    <article id={`post-${post.id}`} className="rounded-2xl border border-border bg-card p-5 shadow-card transition-all hover:shadow-elegant">
            ...
            <div key={c.id} className="flex gap-2">
              <Avatar className="h-7 w-7">
                <AvatarFallback className="text-xs">
                  {(c.profiles?.full_name || c.profiles?.username || "م").charAt(0)}
                </AvatarFallback>
              </Avatar>
              <div className="flex-1 rounded-xl bg-muted px-3 py-2">
                <div className="flex items-start justify-between gap-2">
                  <p className="text-xs font-semibold">{c.profiles?.full_name || c.profiles?.username}</p>
                </div>
