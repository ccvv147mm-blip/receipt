...
function AuthedHome() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("home-store-open");
    if (saved !== null) setStoreOpen(saved === "1");
  }, []);
  ...
  const listings = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("listings").select("id, title, price_points, images, kind").eq("status", "active").order("created_at", { ascending: false }).limit(6);
      return data ?? [];
    },
  });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-2xl px-4 py-6 space-y-4">
                  ...
                  <Link key={l.id} to="/marketplace/$listingId" params={{ listingId: l.id }} className="shrink-0 w-32">
                    <div className="overflow-hidden rounded-xl border border-border">
                      ...
                      <div className="p-2">
                        ...
                        <p className="mt-0.5 flex items-center gap-1 text-[11px] text-amber-500"><Coins className="h-3 w-3" />{l.price_points}</p>
                      </div>
                    </div>
                  </Link>
                ))}
        ...
        {videos.data && videos.data.length > 0 && (
          <section className="rounded-2xl border border-border bg-card p-4 shadow-card">
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-center gap-2"><Video className="h-4 w-4 text-primary" /><h2 className="text-sm font-bold">فيديوهات حديثة</h2></div>
