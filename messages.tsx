              ...
              <button onClick={() => onOpen(u.id)} className="flex flex-1 items-center gap-3 text-start">
                <Avatar className="h-9 w-9">
                  ...
                  <AvatarFallback className="gradient-primary text-sm font-bold text-primary-foreground">
                    {(u.full_name || u.username || "م").charAt(0)}
                  </AvatarFallback>
                </Avatar>
        ...
        {list.data?.conversations.map((c) => (
          ...
          >
            <Avatar className="h-11 w-11">
              ...
              <AvatarFallback className="gradient-primary text-sm font-bold text-primary-foreground">
                {(c.peer?.full_name || c.peer?.username || "م").charAt(0)}
              </AvatarFallback>
            </Avatar>
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-2">
                <p className="truncate font-semibold">{c.peer?.full_name || c.peer?.username || "مستخدم"}</p>
    ...
    <div className="flex h-[calc(100vh-5rem)] flex-col">
      <header className="flex items-center gap-2 border-b border-border pb-3">
        ...
        <Avatar className="h-9 w-9">
          ...
          <AvatarFallback className="gradient-primary text-sm font-bold text-primary-foreground">
            {(peer?.full_name || peer?.username || "م").charAt(0)}
          </AvatarFallback>
        </Avatar>
        <div>
          <p className="font-semibold">{peer?.full_name || peer?.username || "..."}</p>
          {peer?.username && <p className="text-xs text-muted-foreground">@{peer.username}</p>}
        </div>
      </header>
